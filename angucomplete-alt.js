...
angular.module('angucomplete-alt', [] ).directive('angucompleteAlt', ['$q', '$parse', '$http', '$sce', '$timeout', function ($q, $parse, $http, $sce, $timeout) {
  ...
  return {
    ...
    template:
      ...
      '  <input id="{{id}}_input" ng-model="searchStr" type="text" placeholder="{{placeHolder}}" class="{{inputClass}}" ng-focus="resetHideResults()" ng-blur="hideResults()" autocapitalize="off" autocorrect="off" autocomplete="off" ng-change="inputChangeHandler(searchStr)"/>' +
      ...
      function getLocalResults(str) {
        ...
        for (i = 0; i < scope.localData.length; i++) {
          ...
          for (s = 0; s < searchFields.length; s++) {
            if(!scope.localData[i][searchFields[s]])scope.localData[i][searchFields[s]]='';
      ...
      scope.processResults = function(responseData, str) {
        ...
        if (responseData && responseData.length > 0) {
          ...
          for (i = 0; i < responseData.length; i++) {
            ...
            image = '';
            if (scope.imageField) {
              image = 'Images/'+extractValue(responseData[i], scope.imageField)+'.png';
            }
